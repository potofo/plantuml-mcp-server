...
- **[RESULTS.md](RESULTS.md)** — markdown summary regenerated by every
  run of the script (date, image, commit, per-requirement table). The
  committed copy is the snapshot of the latest verified run.
- **CI artifact `test-results`** — every CI run uploads the raw TAP
  output (`acceptance-tap.txt`) and `RESULTS.md`, downloadable from the
  run's Summary page even when the step fails.
