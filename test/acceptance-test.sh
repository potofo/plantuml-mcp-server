...
{
  echo "# Acceptance Test Results / 受け入れテスト結果"
  echo
  echo "> Generated by \`test/acceptance-test.sh\` on every run — do not edit by hand."
  echo "> このファイルは \`test/acceptance-test.sh\` が実行のたびに再生成します(手動編集不可)。"
  echo "> Requirements / 要件定義: [ACCEPTANCE.md](ACCEPTANCE.md) / [ACCEPTANCE.ja-JP.md](ACCEPTANCE.ja-JP.md)"
  echo
