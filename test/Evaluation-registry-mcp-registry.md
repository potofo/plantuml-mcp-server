...
>
> Snapshot date: 2026-08-09 (branch `fix/registry-review-items`, PR #9)
...
| 6 | Testing | 88 | 8 JUnit tests (rendering, validation, concurrency regression) + 23-item acceptance suite tracing every EARS requirement ID to a TAP check, run in CI against the real image with `--network none`. Not automated: the 60s timeout path (design-reviewed) and multi-arch verification (deferred to the release run). |
| 7 | Documentation | 92 | Bilingual README/requirements, architecture diagram, security section, design and license notes, research notes covering local/WSL/remote test flows and the registration procedure itself. |
...
- Acceptance suite: [`test/acceptance-test.sh`](acceptance-test.sh),
  requirements in [`test/ACCEPTANCE.md`](ACCEPTANCE.md) — latest local
  run: 21 passed, 0 failed, 2 skipped (by design).
- CI runs: `CI` workflow on `main` and PR #9 (build + tests + acceptance).
